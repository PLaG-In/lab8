...
__int64 g_freq, show_time, find_time; // глобальная переменная
...
map<string, int> FindCommonWord(string line)
{
	...
	find_time = GetMicroTickCount() - begin;
	return Dictionary;
}
...
char* ShowDictionary(map<string, int> Dictionary)
{
	...
	show_time = GetMicroTickCount() - begin;
	return Result;
}

char* Deleted_Comment(char* str)
{
	__int64 begin = GetMicroTickCount();
	assert(str);
	char* t = str;
...
int main(int argc, char* argv[])
{
	...
	cout << "Total time of ShowDictionary function(mcs) : " << show_time << endl;
	cout << "Total time of FindCommonWord function(mcs) : " << find_time << endl;
	cout << "Total time of Deleted_Comment function(mcs): " << summ << endl;
	cout << "Average time of Deleted_Comment function(mcs): " << average << endl;
